#!/usr/bin/env python3
"""
Workflow Analyzer - Uses Official Categories from def_categories.json
...
class WorkflowAnalyzer:
    """Analyze and process n8n workflows using official categories."""
    ...
    def __init__(self, workflows_dir: str = "workflows"):
        ...
        self.integration_to_category = {}
        self._load_official_categories()
    
    def _load_official_categories(self):
        """Load official category mapping from def_categories.json"""
        try:
            with open('context/def_categories.json', 'r', encoding='utf-8') as f:
                categories_list = json.load(f)
            
            # Build integration -> category mapping
            for item in categories_list:
                integration = item.get('integration', '').lower()
                category = item.get('category', 'Business Process Automation')
                if integration:
                    self.integration_to_category[integration] = category
            
            print(f"✅ Loaded {len(self.integration_to_category)} official integrations")
        except FileNotFoundError:
            print("❌ context/def_categories.json not found")
            raise
    ...
    def _extract_integration(self, node_type: str, node_name: str) -> str:
        ...
        # Check for known integrations (official ones first)
        for integration_name in self.integration_to_category.keys():
            if integration_name in combined:
                # Return title case version
                return combined.split(integration_name)[1].split()[0] if integration_name in combined else integration_name.title()
        ...
        if 'n8n-nodes' in node_type:
            ...
            if len(parts) > 1:
                integration = parts[-1]
                # Check if this integration is in our official list
                if integration.lower() in self.integration_to_category:
                    return integration.title()
                if len(integration) > 2:
                    return integration.title()
        
        # Extract from node_name as fallback
        if node_name and len(node_name) > 2:
            first_word = node_name.split()[0]
            if first_word.lower() in self.integration_to_category:
    ...
    def categorize_workflow(self, integrations: List[str], filename: str) -> str:
        """Categorize workflow based on integrations using official mapping."""
        # First try to find category from integrations using official mapping
        for integration in integrations:
            integration_lower = integration.lower()
            if integration_lower in self.integration_to_category:
                return self.integration_to_category[integration_lower]
            
            # Try partial match
            for official_int, category in self.integration_to_category.items():
                if official_int in integration_lower or integration_lower in official_int:
                    return category
        
        # Fallback: try filename-based categorization
        filename_lower = filename.lower()
        for official_int, category in self.integration_to_category.items():
            if official_int in filename_lower:
                return category
    ...
    def generate_description(self, metadata: Dict[str, Any]) -> str:
        ...
        # Build description
        if integrations:
            integration_str = ' and '.join(integrations[:3])  # Show first 3
            if len(integrations) > 3:
                integration_str += f" and {len(integrations)-3} more"
    ...
    def process_all_workflows(self) -> Dict[str, Any]:
        ...
        for file_path_str in workflow_files:
            ...
            # Categorize using official categories
    ...
    def generate_report(self) -> str:
        ...
        report = []
        report.append("=" * 80)
        report.append("N8N WORKFLOW ANALYSIS REPORT (OFFICIAL CATEGORIES)")
        report.append("=" * 80)
        ...
        for category, count in sorted_categories:
            report.append(f"{category:.<45} {count:>4} workflows")
        ...
        for category in sorted(self.workflows.keys()):
            ...
            for workflow in sorted(workflows_in_cat, key=lambda w: w['filename'])[:10]:
                report.append(f"\n  📄 {workflow['filename']}")
                ...
                if workflow['integrations']:
                    report.append(f"     🔌 Integrations: {', '.join(workflow['integrations'][:5])}")
            
            if len(workflows_in_cat) > 10:
                report.append(f"\n  ... and {len(workflows_in_cat)-10} more workflows in this category")
    ...
    def update_workflow_descriptions(self, save_backup: bool = True):
        """Update workflow JSON files with new descriptions using official categories."""
        updated_count = 0
        ...
        for category, workflows_list in self.workflows.items():
            for workflow_meta in workflows_list:
                ...
                try:
                    ...
                    # Save backup if it doesn't exist already
                    backup_path = file_path.with_suffix('.json.bak')
                    if not backup_path.exists():
                        with open(backup_path, 'w', encoding='utf-8') as f:
                            json.dump(workflow, f, indent=2)
                    
                    # Update metadata section with official category
                    if 'meta' not in workflow:
                        workflow['meta'] = {}
                    
                    workflow['meta']['description'] = workflow_meta['description']
                    workflow['meta']['category'] = workflow_meta['category']  # Official category
...
def main():
    ...
    print("\n🔍 ANALYZING ALL WORKFLOWS (USING OFFICIAL CATEGORIES)")
    print("=" * 80)
    ...
    # Update workflow descriptions with OFFICIAL categories
    print(f"\n📝 UPDATING WORKFLOW DESCRIPTIONS WITH OFFICIAL CATEGORIES")
    print("=" * 80)
    updated = analyzer.update_workflow_descriptions(save_backup=False)
    print(f"\n✅ Updated {updated} workflow files with official categories")
